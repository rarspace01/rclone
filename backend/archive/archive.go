// Package archive implements a backend to archive multiple remotes in a directory tree
package archive

/*
   Have API to add/remove branches in the archive
*/
...
func (f *Fs) List(ctx context.Context, dir string) (entries fs.DirEntries, err error) {
	...
	for archiverRemote, archive := range f.archives {
		...
		if strings.HasPrefix(dirSlash, subRemote) {
			listF, err = archive.init(ctx, f.f)
			if err != nil {
				return nil, err
			}
			break
	...
	entries, err = listF.List(ctx, dir)
	if err != nil {
		return nil, err
	}
...
func (f *Fs) NewObject(ctx context.Context, remote string) (fs.Object, error) {
	o, err := f.f.NewObject(ctx, remote)
	if err != nil {
		return nil, err
	}
	return o, nil
